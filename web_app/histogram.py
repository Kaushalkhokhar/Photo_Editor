...
def histogram_plot():
    ...
    method = file_processed[0][:(len(file_processed[0]) - len(file_source[0]))] #To return the current method to html doc
    
    #image  = cv2.imread(r"D:\Programming\Python\Flask\Web_App\src\web_app\processed_images\bilateral_blurfruits.png")
    image  = cv2.imread(processed_image_path, cv2.IMREAD_COLOR)
